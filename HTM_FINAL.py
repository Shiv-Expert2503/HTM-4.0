...
# # Handeling Translate command
# async def translate(update: Update, context: ContextTypes.DEFAULT_TYPE):
#     await context.bot.send_message(chat_id=update.effective_chat.id, text="Enter the text")
#     try:
#         author = update.message.from_user.first_name
#         logging.info(f"Translate command activated by {author}")
...
#         await context.bot.send_message(chat_id=update.effective_chat.id, text=f"{translated_text.text}")
#     except Exception as e:
#         logging.error(f"Error translating {update.message.text}", exc_info=context.error)
#
#
# # Handeling Voice Translate command
...
#                     pipe = pipeline("translation", model="Helsinki-NLP/opus-mt-hi-en")
#                     print(pipe(translated_text.text)[0]['translation_text'])
#                     await context.bot.send_message(chat_id=update.effective_chat.id, text=f"You said ==>{reply}...And here is the translated voice ==>{translated_text.text}")
#                 except Exception as e:
#                     logging.error(f"Error translating {update.message.text}", exc_info=context.error)
...
#
# # Handeling Audio Translate command
...
def main():
    ...
    # # Making command handler for translate_voice
    # application.add_handler(CommandHandler('translate_voice', translate_voice))
    # # Making command handler for translate_audio
    # application.add_handler(CommandHandler('translate_audio', translate_audio))
